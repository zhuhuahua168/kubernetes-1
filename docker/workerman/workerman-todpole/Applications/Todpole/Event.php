        ...
        switch ($message_data['type']) {
			...
			case 'skiper_say':
				...
				if($message_data['to_client_id'] != 'all'){
					...
					$content['types'] = $content['types'] ? $content['types'] : 2;
					$content['voice'] = $content['voice'] ? $content['voice'] : array('voiceUrl'=>'','voiceLength'=>'');
					...
					$insert_id=$db1->insert('user_chat_log')->cols( array(
			...
			case 'skiper_roomsay':
				...
				if($all_clients){
					...
					// 获取所有数据
					$userId=$db1->select('uid,circleid')->from('circle_user')->where('circleid= :circleid')->bindValues(array('circleid'=>$room_id))->query();
					...
					//insert into chat table
					$insert_id =$db1->insert('circle_chat_log')->cols( array(
							'uid' => $message_data['from_client_id'],
							'circleid'  => $room_id,
							'content' => json_encode($content),
							'createtime'=>time()
